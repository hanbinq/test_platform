...
def api_debug(request):
    ...
    if request.method == "POST":
        ...
        if method == "get":
            if type_ == "from":
                r = requests.get(url, params=payload, headers=header_dict)
            else:
                return common.response_failed("参数类型错误")

        if method == "post":
            if type_ == "from":
                r = requests.post(url, data=payload, headers=header_dict)
            elif type_ == "json":
                r = requests.post(url, json=payload, headers=header_dict)
            else:
                return common.reponse_failed("参数类型错误")
...
def get_case_info(request):
    ...
    if request.method == "POST":
        ...
        if case_id == "":
            return common.reponse_failed("用例id为空")
